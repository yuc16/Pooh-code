...
TOOL_DESCRIPTION_MAP = {
    ...
    "web_fetch": "已知具体 URL 时抓取网页正文（Jina Reader 优先，BS4 兜底），SPA 页面也能解析。",
    "web_search": "智能多引擎联网搜索：根据 query 特征自动路由（中文→Bocha+Tavily、神经/论文→Exa、新闻→Tavily+Brave、默认→Tavily+Brave+Exa），合并去重并标记来源。",
    "deep_research": "深度研究：Jina DeepSearch 迭代式 search→read→reason 产出带引用的答案；不可用时回退到多引擎搜索 + Reader 抓正文。",
    "paper_search": "通过 OpenAlex 检索学术论文，返回标题、作者、年份、期刊、DOI、被引数和开放获取链接等结构化结果。",
