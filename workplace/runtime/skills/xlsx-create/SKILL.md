---
name: xlsx-create
description: 生成或编辑 Excel 电子表格（.xlsx/.csv 文件）。当用户提到"表格""电子表格""Excel""xlsx""csv""数据分析""财务模型""排班表""统计报表"等需要创建或处理表格数据时使用此 skill。适用于从零创建带格式的 Excel 文件、分析现有数据并输出结果、生成带公式和图表的专业表格。只要最终交付物是 .xlsx 或 .csv 文件就应使用。不用于 Word 文档、PPT 或其他格式。
---
...
wb.save('workplace/output/数据表.xlsx')
```
...
# 输出到 Excel（简单输出，无格式）
df.to_excel('workplace/output/分析结果.xlsx', index=False)
```
...
wb.save('workplace/output/修改后.xlsx')
```
