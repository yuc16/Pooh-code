...
2. 用 `write_file(path="scripts/...")` 编写 Python 脚本生成 .pptx
...
prs.save(f'output/{session_id}/演示文稿.pptx')
```
...
```python
# 插入本地图片
slide.shapes.add_picture(f'output/{session_id}/chart.png', Cm(18), Cm(4), Cm(14), Cm(10))
```
...
ax.set_title('季度营收')
fig.savefig(f'output/{session_id}/chart.png', dpi=150, bbox_inches='tight', transparent=True)
plt.close()

# 然后在 PPT 中插入
slide.shapes.add_picture(f'output/{session_id}/chart.png', Cm(17), Cm(4), Cm(15))
```
...
prs.save(f'output/{session_id}/修改后.pptx')
```
