...
当 agent 通过 `write_file` 或 `bash` 在 `workplace/output/` 下生成文件时，前端会自动检测并在助手气泡末尾显示下载按钮（带文件图标和文件名）。点击按钮即可在浏览器中直接下载。

支持的文件类型包括 Office 三件套（.docx/.xlsx/.pptx）、PDF、CSV、图片、文本等。下载通过 `/api/download?path=<相对路径>` 端点实现，服务端自动设置正确的 MIME 类型和 `Content-Disposition` 头。
...
| GET  | `/api/files` | 列出 `workplace/output/` 下的所有文件（跳过 `.git` 等隐藏目录），按修改时间降序 |
| GET  | `/api/download?path=<rel>` | 下载 `workplace/output/` 中的文件，自动设置 MIME 类型和 `Content-Disposition`；Office 文件（.docx/.xlsx/.pptx）触发浏览器下载 |
