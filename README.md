...
- 每个会话条目显示中文标题，标题下方附带小字 `session_id`
...
- 左侧“产物归档”面板按 `session_id` 展示该会话目录下的所有文件；Office 文件、中间脚本、图片等都可直接下载
...
| POST | `/api/chat/stream` | **SSE 流式**接口；请求体里显式带 `session_id`，不同会话可并行跑，同一会话只允许一个运行中的任务 |
