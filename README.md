...
- 聊天框底部新增了一个**图片生成**模式按钮；开启后发送入口不再走 `PoohAgent.ask_stream`，而是改调 AIHubMix 的图片模型。现在支持**多模型切换**：`workplace/runtime/config/settings.json` 里 `image.models` 数组列出可用模型（默认顺序 `web-gpt-image-2-vip`（默认选中） + `gemini-3.1-flash-image-preview-free`），`image.model` 是默认选中项。前端会把该列表吐到 `GET` 状态里的 `image_generation.models`；**模型切换入口挂在右上角的 model 徽章里**（徽章末尾一个 `▾` 下拉，点击弹出原生 select 菜单；只有在图片生成模式 **且** 列表 ≥ 2 项时才显示），不占用 composer 空间。用户选中的模型随 `POST /api/image/generate` 的 `model` 字段回传，后端 `image_generation.py` 会先尝试解析 Gemini 风格的 `multi_mod_content` 内联 base64，若为空则从 `choices[0].message.content` 文本里正则抽取图片 URL（`.png/.jpg/.jpeg/.gif/.webp/.bmp/.svg`）；针对 `web-gpt-image-2-vip` 这类把"预览 + 下载"两次引用同一张图的 GPT 反代模型，URL 回退路径**只下载第一张**避免重复产物，并把 `![](url)` / `[点击下载](url)` 等 markdown 残渣一并清洗，不再在正文里漏出 `!()` 乱码。顶栏模型徽章会立即切到当前选中的图片模型名，关闭该模式后恢复显示主文本模型 `gpt-5.4`
