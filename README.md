...
- 配套地，前端 `/api/command` 同步分支返回时会先比对 `state.sessionId` 是否还是命令发起时的那个会话；如果用户已经切到其它会话了，就只静默刷新侧栏，不再用响应里的 `session_id` 强行覆盖当前视图，避免把用户从新会话"拽回"原会话造成"新会话变成原会话"的错觉
...
- **侧栏折叠**：左栏顶栏刷新按钮旁 / 右栏顶栏左侧各有一个折叠按钮（chevron 图标），点击即可把对应侧栏收起，中栏自动铺满剩余宽度；折叠后在屏幕左/右边缘会出现一条贴边的半圆展开手柄，点击恢复；左右两侧折叠状态独立持久化到 `localStorage.pooh.collapse.v1`
- 左侧栏：顶部是品牌 logo + 刷新按钮 + 搜索框（⌘K 聚焦）+ "新建会话"按钮（⌘N 新建）、会话列表（`.convo` 行）、底部用户卡片（头像 + 邮箱 + 退出按钮）
- 首页左上角和登录页品牌区继续使用 [src/frontend/static/logo.jpg](/Users/wangyc/Desktop/projects/Pooh-code/src/frontend/static/logo.jpg)；浏览器页签 favicon 与 touch icon 单独使用按新构图裁好的 [src/frontend/static/favicon-pooh.png](/Users/wangyc/Desktop/projects/Pooh-code/src/frontend/static/favicon-pooh.png)，避免直接拿页面 logo 缩小后在页签里变形或显得发丑；首页和登录页左上角 logo 不再带边框、底色或阴影方框，并统一带版本参数以尽量绕开浏览器 favicon 缓存
...
- **多轮工具调用二级折叠**：当一段连续出现 ≥2 条纯工具 assistant 消息（或单条 assistant 气泡里出现多个 thinking-group）时，整段会再外裹一层「已处理 N 个工具调用 · 展开详情」的 mega-fold，默认收起；点击展开后才露出原先的每一条「已处理 / N 个工具调用」卡片，再点单条可看具体参数/输出。目的：避免模型最终回复之前被成片的工具卡片挤出屏幕
...
- 聊天框底部新增了一个**图片生成**模式按钮；开启后发送入口不再走 `PoohAgent.ask_stream`，而是按选中的模型分发到不同的图像后端。现在支持**多模型 + 多 provider**：`workplace/runtime/config/settings.json` 的 `image` 段配置两套 provider 凭据 —— `api_key`/`base_url`（aihubmix，chat completions 反代图像模型）和 `apimart_api_key`/`apimart_base_url`（apimart，原生 `/v1/images/generations`）；`image.models` 数组列出可选模型（默认 `web-gpt-image-2-vip`（默认选中） / `gemini-3.1-flash-image-preview-free` / `gpt-image-2`），`image.default_aspect_ratio` 与 `image.default_resolution` 提供启动默认值。
- 每个图片模型在 `pooh_code/image_generation.py` 顶部的 `MODEL_CAPABILITIES` 里登记了三项能力：`kind`（`chat` 走 aihubmix 同步链路 / `apimart` 走异步任务 + 轮询）、`ratios`（该模型实际支持的比例白名单，前端下拉按它动态重建）、`resolutions`（仅 apimart 系模型有 `1k`/`2k`/`4k` 真清晰度参数）、`supports_reference`（是否能把上一张生成图作为下一次的 `image_urls` 参考实现"图生图记忆"）。后端调用 `generate_images()` 时按 `kind` 分发到 `_generate_via_chat`（同步）或 `_generate_via_apimart`（POST `/images/generations` 拿 `task_id`，轮询 `/tasks/<task_id>`，超时 5 分钟），从 `result.images[].url` 取第一张下载到 `workplace/output/<session_id>/`。
- 前端配套：composer 上的下拉**只显示当前选中模型支持的能力**——比例下拉按 `caps.ratios` 重建（GPT 反代只给 1:1/2:3/3:2，Gemini 给 10 种，gpt-image-2 给 14 种含 `2:1`/`9:21`/`auto`）；清晰度下拉只在模型有 `resolutions` 时（即 gpt-image-2）显示。**模型切换入口在右上角 model 徽章末尾 `▾`**，仅图片模式 + 模型 ≥ 2 时显示。
- **图生图（伪记忆）**：当 `caps.supports_reference=true`（目前只有 gpt-image-2），图片生成模式下每张已生成图的卡片右上角会浮现一个 ↻ 按钮，点击后该图被记为"下一次的参考图"，composer 在 textarea 上方出现一条"基于：image-gen-XX.png"暖色徽章；下次发送时前端把该图相对路径放进 `POST /api/image/generate` 的 `reference_image_paths`，后端读盘转 base64 data URI 后塞进 apimart 请求的 `image_urls` 数组（最多 16 张，模型在原图基础上修改）。徽章上的 × 可清除参考。
- **切到 gpt-image-2 时弹软提示**：`image-model` 下拉一旦切到 `kind=apimart` 的模型，前端弹**自定义暖白卡片对话框**（参考 Claude.ai 风格，`#confirm-dialog`，`Esc`/点背景取消、`Enter` 确认、聚焦在主按钮、半透明 backdrop blur）—— 点"新建会话"立即调 `/api/session/new` 新开会话**并把刚选中的模型回填进去**（fix 服务端 `applyState` 把 `image_generation.model` 重置为配置默认值的 bug）；点"继续在当前会话"就地继续。这是软建议、不强制。
- **图片偏好按 session 持久化**：服务端 `image.model`/`default_aspect_ratio`/`default_resolution` 是全局配置默认值，每次 `/api/state` 拉到都会被 `applyState` 写回 `state`。为了让用户在某个会话里选过的模型/比例/清晰度/参考图刷新后不被打回，前端用 localStorage 按 `pooh.image.<session_id>` 存这一组偏好：用户每改一次 model/aspect/resolution/参考图都会触发 `saveSessionImagePrefs()`；`applyState` 在写完服务端默认值之后立即调 `loadSessionImagePrefs(sessionId)` 把本地偏好覆盖回去。新建会话时也会把刚选中的偏好同步进新 session 的 storage 项，避免下一次刷新又回退。
- 顶栏模型徽章实时显示当前选中的图片模型名，关闭图片模式后恢复显示主文本模型 `gpt-5.4`。chat-completions 路径（aihubmix）针对 `web-gpt-image-2-vip` 这类把"预览 + 下载"两次引用同一张图的 GPT 反代模型，URL 回退路径**只下载第一张**避免重复产物，并清洗 `![](url)` / `[点击下载](url)` 等 markdown 残渣，不再漏出 `!()` 乱码
...
- 输入框：Enter 发送，Shift+Enter 换行，⌘↵ 发送；textarea 高度自动增长；附件栏与引用条会自动收起/展开；composer 下方一排 chip 提供 `图片生成` `/tools` `/skills` 快捷入口，其中 slash 命令 **点击即直接发送执行**（`/help`、`/clear`、`/compact` 仍可手动输入使用，只是不再占用 chip 条位置）；`图片生成` 是一个可切换模式，启用后 placeholder 会变成“描述你想生成的图片…”，同时在按钮右侧出现**图片比例**下拉（`1:1`/`2:3`/`3:2`/`3:4`/`4:3`/`4:5`/`5:4`/`9:16`/`16:9`/`21:9`，默认读取 `workplace/runtime/config/settings.json` 的 `image.default_aspect_ratio`），用户选中的比例会作为 `aspect_ratio` 附加在 `POST /api/image/generate` 的请求体里；并把同一会话里的生成结果同时写进聊天区和左栏该 session 的产物归档；**agent 运行时输入框保持可用**——用户可随时输入并发送「插话」消息（走 `POST /api/session/inject`），消息会被推入运行中 session 的注入队列，agent 在当前工具执行完、下一轮 LLM 调用前自动读取并追加到 transcript，SSE 流会发一条 `injected` 事件通知前端（在助手气泡中内联显示琥珀色 USER 标签 + 插话内容）；placeholder 会切换为"继续发送消息，Agent 将在下一轮看到"提示；运行时也仍有 `停止` 按钮可用
